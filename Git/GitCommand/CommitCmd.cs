...
namespace gsi
{
    partial class GitCommand
    {
        public static void CommitCmd(string message, List<string> included, List<string> excluded)
        {
            ...
            if (gitfs.tmp == null)
                gitfs.tmp = new Tmp(gitfs, false);

            (var lmer, var lch, var lnew, var ldel) = gitfs.TrackWorkingCopy();
            gitfs.track.WriteTrack();

            gitfs.tmp.SetEntriesFromTrack();
            gitfs.tmp.WriteTmp();

            // set temporary, without being written
            ...
            var add = lch.Union(lnew).Append(".track").Append(".tmp");
            var del = ldel.Union(gitfs.track.tmp);
